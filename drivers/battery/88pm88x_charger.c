...
static int pm88x_get_charging_status(struct pm88x_charger_info *info)
{
	...
	dev_dbg(info->dev, "%s : PM88X_CHG_CONFIG1 : 0x%02x\n", __func__, cfg1);
	dev_dbg(info->dev, "%s : PM88X_CHG_STATUS1 : 0x%02x\n", __func__, stat1);
...
static int pm88x_charger_init(struct pm88x_charger_info *info)
{
	...
	/* Set RECHG_THR_SET and PM88X_FASTCHG_TOUT_SET */
	regmap_write(info->chip->battery_regmap, PM88X_TIMER_CONFIG, 0x07);
...
static void pm88x_vbus_det_irq_thread(struct work_struct *work)
{
	...
	case POWER_SUPPLY_HEALTH_GOOD:
		...
		if (present.intval) {
			...
			pr_err("%s: Battery missing, maintain UnspecFail status\n", __func__);
		}
		break;
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (charger->is_fullcharged) {
			...
			val->intval = POWER_SUPPLY_STATUS_FULL;
			tst_full_registers = pm88x_get_charging_status(charger);
		} else
			val->intval = pm88x_get_charging_status(charger);
		break;
