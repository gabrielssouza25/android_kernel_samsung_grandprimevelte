...
void pm88x_power_off(void)
{
	...
	if (ta_connected) {
		...
		ret = i2c_raw_update_bits(PM88X_RTC_ALARM_CTRL1, PM88X_WD_PIN_DIS);
		if (ret < 0) {
			pr_err("%s fail to update PM886_RTC_ALARM_CTRL1\n", __func__);
			goto p_off;
		}
		/* 2.set WD timer counter as 1024 ms */
		ret = i2c_raw_update_bits_mask(PM88X_MISC_CONFIG2, PM88X_WD_TIMER_ACT_1S,
				PM88X_WD_TIMER_ACT_MASK);
		if (ret < 0) {
			pr_err("%s fail to update PM886_MISC_CONFIG2\n", __func__);
			goto p_off;
		}
		/* 3.disable FAULT_WU and Enable FAULT_WU_EN */
		ret = i2c_raw_update_bits_mask(PM88X_AON_CTRL7, PM88X_FAULT_WU_EN,
				PM88X_FAULT_WU);
		if (ret < 0) {
		...
		ret = i2c_raw_update_bits(PM88X_MISC_CONFIG1, PM88X_PWR_HOLD);
		if (ret < 0) {
		...
		ret = i2c_raw_write_bits(PM88X_WDOG, PM88X_WD_EN);
		if (ret < 0) {
				   ...
				   unsigned long code, void *cmd)
{
	...
	if (code != SYS_POWER_OFF) {
		...
		/* 1.Enable WD_PIN_DIS field */
		regmap_read(chip->base_regmap, PM88X_RTC_ALARM_CTRL1, &data);
		//pr_info("PM886_RTC_ALARM_CTRL1 Reg(0x%02x) is 0x%02x\n", PM886_RTC_ALARM_CTRL1, data);
		data |= PM88X_WD_PIN_DIS;
		regmap_write(chip->base_regmap, PM88X_RTC_ALARM_CTRL1, data);

		/* 2.set WD timer counter as 4s */
		regmap_read(chip->base_regmap, PM88X_MISC_CONFIG2, &data);
		//pr_info("PM886_MISC_CONFIG2 Reg(0x%02x) is 0x%02x\n", PM886_MISC_CONFIG2, data);
		data &= ~(PM88X_WD_TIMER_ACT_MASK);
		data |= PM88X_WD_TIMER_ACT_4S;
		regmap_write(chip->base_regmap,
				PM88X_MISC_CONFIG2, data);
		//pr_info("0x%02x is written to PM886_MISC_CONFIG2 Reg(0x%02x)\n", data, PM886_MISC_CONFIG2);

		/* 3.disable FAULT_WU and Enable FAULT_WU_EN */
		regmap_read(chip->base_regmap, PM88X_AON_CTRL7, &data);
		//pr_info("PM886_AON_CTRL7 Reg(0x%02x) is 0x%02x\n", PM886_AON_CTRL7, data);
		data &= ~(PM88X_FAULT_WU);
		data |= PM88X_FAULT_WU_EN;
		regmap_write(chip->base_regmap,
				PM88X_AON_CTRL7, data);
		//pr_info("0x%02x is written to PM886_AON_CTRL7 Reg(0x%02x)\n", data, PM886_AON_CTRL7);

		/* 4.Issue SW power down */
		regmap_read(chip->base_regmap, PM88X_MISC_CONFIG1, &data);
		//pr_info("PM886_MISC_CONFIG1 Reg(0x%02x) is 0x%02x\n", PM886_MISC_CONFIG1, data);
		data |= PM88X_PWR_HOLD;
		regmap_write(chip->base_regmap, PM88X_MISC_CONFIG1, data);

		/* 5.enable PMIC WD */
		regmap_write(chip->base_regmap, PM88X_WDOG,
				PM88X_WD_EN);
		regmap_read(chip->base_regmap, PM88X_WDOG, &data);
