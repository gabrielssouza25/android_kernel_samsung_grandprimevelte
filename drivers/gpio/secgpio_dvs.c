...
static ssize_t gpio_sleep_debug_file_write(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t size);
...
static DEVICE_ATTR(gpio_sleep_debug, 0444 , NULL, gpio_sleep_debug_file_write);
	...
	const char *buf, size_t size)
{
	...
	*/
	 if (gpio_ctrl[0] == 1) {
		 if (gdvs->gpio_input_config)
		 {
			 gdvs->gpio_input_config(gpio_ctrl[1], gpio_ctrl[2]);
		 }
		 else
		 {
			 gpio_request(gpio_ctrl[1], "gpio_output_test_on");
		...
		if (gdvs->gpio_output_config)
		{
			gdvs->gpio_output_config(gpio_ctrl[1], gpio_ctrl[2]);
		}
		else
		{
			gpio_request(gpio_ctrl[1], "gpio_output_test_on");
