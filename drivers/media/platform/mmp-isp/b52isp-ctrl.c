...
static int b52isp_ctrl_set_expo_bias(int idx, int id, struct b52_sensor *sensor)
{
	...
	ev_target = (low_def[id] + high_def[id]) >> 1;
	...
	ev_ext = def_slow_step_thresh;
	if (offset < 0 && ev_target != 0)
		ev_ext = def_slow_step_thresh * low_target / ev_target;
...
int b52isp_enlarge_focus_landing_step(int ratio, int id)
{
	u32 base = FW_P1_REG_AF_BASE + id * FW_P1_P2_AF_OFFSET;

	/* to reduce the close time, we need to enlarge step */
	if (ratio > 0)
		b52_writew(base + REG_FW_FOCUS_LANDING_TO_ZERO_STEP,
				b52_readw(base + REG_FW_FOCUS_LANDING_TO_ZERO_POS) / ratio);

	return 0;
}
EXPORT_SYMBOL_GPL(b52isp_enlarge_focus_landing_step);
